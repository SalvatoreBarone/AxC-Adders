...
static int AXA3_sum(int a, int b, int cin);
static int AXA3_carry(int a, int b, int cin);
...
int AXA3_adder(int nab, int a, int b )
{
	...
	for(i = 0; i < nab; i++){
		lsb_a = (a & (1U<<i)) >> i;
		lsb_b = (b & (1U<<i)) >> i;
		lsb_sum = AXA3_sum(lsb_a, lsb_b, current_carry);
		acc |= (lsb_sum << i);
		current_carry = AXA3_carry(lsb_a, lsb_b, current_carry);
	}
	int mask = ~((1U << nab) -1);
