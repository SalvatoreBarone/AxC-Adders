...
static int AMA2_sum(int a, int b, int cin);
static int AMA2_carry(int a, int b, int cin);
...
int AMA2_adder(int nab, int a, int b )
{
	...
	for(i = 0; i < nab; i++){
		lsb_a = (a & (1U<<i)) >> i;
		lsb_b = (b & (1U<<i)) >> i;
		lsb_sum = AMA2_sum(lsb_a, lsb_b, current_carry);
		acc |= (lsb_sum << i);
		current_carry = AMA2_carry(lsb_a, lsb_b, current_carry);
	}
	int mask = ~((1U << nab) -1);
