...
{
    // inexact cell c_in=0
    BOOST_CHECK_EQUAL(AMA1_adder(1,0,0), 0);
    BOOST_CHECK_EQUAL(AMA1_adder(1,0,1), 2);
    BOOST_CHECK_EQUAL(AMA1_adder(1,1,0), 0);
    BOOST_CHECK_EQUAL(AMA1_adder(1,1,1), 2);

    // inexact cell c_in=1
    BOOST_CHECK_EQUAL(AMA1_adder(2,1,1), 2);
    BOOST_CHECK_EQUAL(AMA1_adder(2,1,3), 4);
    BOOST_CHECK_EQUAL(AMA1_adder(2,3,1), 4);
    BOOST_CHECK_EQUAL(AMA1_adder(2,3,3), 6);

}

BOOST_AUTO_TEST_CASE(AMA1_sys)
{
    BOOST_CHECK_EQUAL(AMA1_adder(5,73, 79), 152);
    BOOST_CHECK_EQUAL(AMA1_adder(6,174, 168), 336);
}